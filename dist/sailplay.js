...
exports.default = function (messager) {
    return function (params) {
        ...
        this.gift_purchase = function (gift) {
...
exports.default = function (messager) {
    return function (params) {
        ...
        this.update = function (config) {
            ...
            _sailplayHub2.default.jsonp.get(config.DOMAIN + config.urls.users.info, {
                ...
                if (value.user.birth_date()) {
                    ...
                    value.user.birth_month = _knockout2.default.observable(birth_date.split('-')[1]).extend({ required: true, min: 1, max: 12, pattern: { message: "wrong", params: '^[0-9]{1,2}$' } });
                    ...
                    if (result.vars.length) {
                        _knockout2.default.utils.arrayForEach(result.vars, function (item) {
                            if (item.name == 'child_birthdate') {
                                ...
                                _this.data().user['child_bmonth'](item.value.split('-')[1]);
        ...
        this.popupVm = {
            opened: _knockout2.default.observable(false),
            months: _knockout2.default.observableArray([{ index: 1, name: 'Январь' }, { index: 2, name: 'Февраль' }, { index: 3, name: 'Март' }, { index: 4, name: 'Апрель' }, { index: 5, name: 'Май' }, { index: 6, name: 'Июнь' }, { index: 7, name: 'Июль' }, { index: 8, name: 'Август' }, { index: 9, name: 'Сентябрь' }, { index: 10, name: 'Октябрь' }, { index: 11, name: 'Ноябрь' }, { index: 12, name: 'Декабрь' }]),
            ...
            getField: function getField(field, arr) {
                ...
                if (field == 'child_bmonth') _this.data().user[field].extend({ min: 1, max: 12, pattern: { message: "wrong", params: '^[0-9]{1,2}$' } });
            ...
            next: function next() {
                ...
                if (_this.popupVm.step() == 2) {
                    ...
                    if (!_this.data().user.child_bmonth.isValid()) return;
            ...
            finish: function finish() {
                ...
                if (user.birth_day && user.birth_month && user.birth_year) primary['birthDate'] = user.birth_year + '-' + user.birth_month + '-' + user.birth_day;

                if (user.child_bday && user.child_bmonth && user.child_byear) secondary['child_birthdate'] = user.child_byear + '-' + user.child_bmonth + '-' + user.child_bday;
